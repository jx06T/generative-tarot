...
function Card({ id, handleClick, move, englishName, chineseName, defaultIsFlipped = false, imgUrl = "" }: { id: number, handleClick: Function, defaultIsFlipped?: boolean, move: { x: number, y: number }, englishName: string, chineseName: string, imgUrl?: string, describe: string }) {
    ...
    return (
        <div
            ref={cardRef}
            className={`${defaultIsFlipped ? " duration-1000" : (move.y < -200 ? "duration-700" : " duration-75 ")} transition-all  origin-center card pointer-events-auto absolute select-none mt-3 rounded-2xl  w-[365px] h-[626px] bg-opacity-0 scale-50`}
            // @ts-ignore
